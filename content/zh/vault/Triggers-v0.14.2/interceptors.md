...
- Standalone `Interceptors`, which are instances of the [`ClusterInterceptor`](/vault/Triggers-v0.14.2/clusterinterceptors/) Custom Resource Definition (CRD). You specify these `Interceptors` by referencing them,
...
For more information, see our [example](https://github.com/tektoncd/triggers/tree/release-v0.14.x/examples/github) of using this `Interceptor`.
...
Tekton Triggers ships with the `ClusterInterceptor` Custom Resource Definition (CRD), which you can use to implement custom `Interceptors`. See [`ClusterInterceptors`](/vault/Triggers-v0.14.2/clusterinterceptors/) for more information.
