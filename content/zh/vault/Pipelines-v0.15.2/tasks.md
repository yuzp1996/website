...
- Optional:
  ...
  - [`resources`](#specifying-resources) - **alpha only** Specifies
    [`PipelineResources`](/vault/Pipelines-v0.15.2/resources/) needed or created by your`Task`.
...
- The container image must abide by the [container contract](/vault/Pipelines-v0.15.2/container-contract/).
...
* `/workspace` - This directory is where [resources](#resources) and [workspaces](#workspaces)
  are mounted. Paths to these are available to `Task` authors via [variable substitution](/vault/Pipelines-v0.15.2/variables/)
* `/tekton` - This directory is used for Tekton specific functionality:
    * `/tekton/results` is where [results](#results) are written to.
      The path is available to `Task` authors via [`$(results.name.path)`](/vault/Pipelines-v0.15.2/variables/)
    * There are other subfolders which are [implementation details of Tekton](https://github.com/tektoncd/pipeline/tree/release-v0.15.x/docs/developers/README.md#reserved-directories)
...
a [`PipelineResources`](/vault/Pipelines-v0.15.2/resources/#using-resources) entity.
...
[`Workspaces`](/vault/Pipelines-v0.15.2/workspaces/#using-workspaces-in-tasks) allow you to specify
...
For more information, see [Using `Workspaces` in `Tasks`](/vault/Pipelines-v0.15.2/workspaces/#using-workspaces-in-tasks)
and the [`Workspaces` in a `TaskRun`](https://github.com/tektoncd/pipeline/tree/release-v0.15.x/examples/v1beta1/taskruns/workspace.yaml) example YAML file.
...
The stored results can be used [at the `Task` level](/vault/Pipelines-v0.15.2/pipelines/#configuring-execution-results-at-the-task-level)
or [at the `Pipeline` level](/vault/Pipelines-v0.15.2/pipelines/#configuring-execution-results-at-the-pipeline-level).
...
- [Mount a Kubernetes `Secret`](/vault/Pipelines-v0.15.2/auth/).
...
For further information, see [`Sidecars` in `TaskRuns`](/vault/Pipelines-v0.15.2/taskruns/#specifying-sidecars).
...
Also see the [complete list of variable substitutions for Tasks](/vault/Pipelines-v0.15.2/variables/#variables-available-in-a-task).
...
- To reference a parameter in a `Task`, use the following syntax, where `<name>` is the name of the parameter:
  ...
  $(params.<name>)
  ```
- To access parameter values from resources, see [variable substitution](/vault/Pipelines-v0.15.2/resources/#variable-substitution)
...
The example Task/TaskRun above can be found as a [TaskRun example](https://github.com/tektoncd/pipeline/tree/release-v0.15.x/examples/v1beta1/taskruns/run-steps-as-non-root.yaml).
