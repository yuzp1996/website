...
- [Events](/vault/Pipelines-v0.14.3/events/#pipelineruns)



## Overview

A `PipelineRun` allows you to instantiate and execute a [`Pipeline`](/vault/Pipelines-v0.14.3/pipelines/) on-cluster.
...
- Required:
  ...
  - [`spec`][kubernetes-overview] - Specifies the configuration information for
    ...
    - [`pipelineRef` or `pipelineSpec`](#specifying-the-target-pipeline) - Specifies the target [`Pipeline`](/vault/Pipelines-v0.14.3/pipelines/).
- Optional:
  - [`resources`](#specifying-resources) - Specifies the [`PipelineResources`](/vault/Pipelines-v0.14.3/resources/) to provision
  ...
  - [`taskRunSpec`](#specifying-task-run-specs) - Specifies a list of `PipelineRunTaskSpec` which allows for setting `ServiceAccountName` and [`Pod` template](/vault/Pipelines-v0.14.3/podtemplates/) for each task. This overrides the `Pod` template set for the entire `Pipeline`. 
  - [`timeout`](#configuring-a-failure-timeout) - Specifies the timeout before the `PipelineRun` fails.
  - [`podTemplate`](#pod-template) - Specifies a [`Pod` template](/vault/Pipelines-v0.14.3/podtemplates/) to use as the basis
...
The `Pipeline` in the [`pipelineSpec` example](https://github.com/tektoncd/pipeline/tree/release-v0.14.x/examples/v1beta1/pipelineruns/pipelinerun-with-pipelinespec.yaml)
...
In the [`taskSpec` in `pipelineSpec` example](https://github.com/tektoncd/pipeline/tree/release-v0.14.x/examples/v1beta1/pipelineruns/pipelinerun-with-pipelinespec-and-taskspec.yaml)
...
For more information, see [`ServiceAccount`](/vault/Pipelines-v0.14.3/auth/).
...
You can specify a [`Pod` template](/vault/Pipelines-v0.14.3/podtemplates/) configuration that will serve as the configuration starting
...
wide `ServiceAccountName`  and [`podTemplate`](/vault/Pipelines-v0.14.3/podtemplates/) configuration,
for example:
...
- For information on mapping `Workspaces` to `Volumes`, see [Specifying `Workspaces` in `PipelineRuns`](/vault/Pipelines-v0.14.3/workspaces/#specifying-workspaces-in-pipelineruns).
- For a list of supported `Volume` types, see [Specifying `VolumeSources` in `Workspaces`](/vault/Pipelines-v0.14.3/workspaces/#specifying-volumesources-in-workspaces).
- For an end-to-end example, see [`Workspaces` in a `PipelineRun`](https://github.com/tektoncd/pipeline/tree/release-v0.14.x/examples/v1beta1/pipelineruns/workspaces.yaml).
...
For more information, see the [`LimitRange` code example](https://github.com/tektoncd/pipeline/tree/release-v0.14.x/examples/v1beta1/pipelineruns/no-ci/limitrange.yaml).
...
[`config/config-defaults.yaml`](https://github.com/tektoncd/pipeline/tree/release-v0.14.x/config/config-defaults.yaml).
...
to a `TaskRun`, the complete [status of the `TaskRun`](/vault/Pipelines-v0.14.3/taskruns/#monitoring-execution-status) and details
...
When a `PipelineRun` changes status, [events](/vault/Pipelines-v0.14.3/events/#pipelineruns) are triggered accordingly.
