...
- [Events](/vault/Pipelines-v0.27.3/events/#taskruns)
- [Running a TaskRun Hermetically](/vault/Pipelines-v0.27.3/hermetic/)
...
A `TaskRun` allows you to instantiate and execute a [`Task`](/vault/Pipelines-v0.27.3/tasks/) on-cluster. A `Task` specifies one or more
...
- Optional:
  ...
  - [`podTemplate`](#specifying-a-pod-template) - Specifies a [`Pod` template](/vault/Pipelines-v0.27.3/podtemplates/) to use as
  ...
  - [`workspaces`](#specifying-workspaces) - Specifies the physical volumes to use for the
    [`Workspaces`](/vault/Pipelines-v0.27.3/workspaces/#using-workspaces-in-tasks) declared by a `Task`.
...
configmap, see [`install.md`](/vault/Pipelines-v0.27.3/install/#customizing-the-pipelines-controller-behavior)**
...
A working example can be found [here](https://github.com/tektoncd/pipeline/tree/release-v0.27.x/examples/v1beta1/taskruns/no-ci/tekton-bundles.yaml).
...
the artifact adheres to the [contract](/vault/Pipelines-v0.27.3/tekton-bundle-contracts/). Additionally, you may also use the `tkn`
cli *(coming soon)*.

### Specifying `Parameters`

If a `Task` has [`parameters`](/vault/Pipelines-v0.27.3/tasks/#parameters), you can use the `params` field to specify their values:
...
If a `Task` requires [`Resources`](/vault/Pipelines-v0.27.3/tasks/#specifying-resources) (that is, `inputs` and `outputs`) you must
specify them in your `TaskRun` definition. You can specify `Resources` by reference to existing
[`PipelineResource` objects](/vault/Pipelines-v0.27.3/resources/) or embed their definitions directly in the `TaskRun`.
...
**Note:** You can use the `paths` field to [override the paths to a `Resource`](/vault/Pipelines-v0.27.3/resources/#overriding-where-resources-are-copied-from).

### Specifying `Resource` limits

Each Step in a Task can specify its resource requirements. See
[Defining `Steps`](/vault/Pipelines-v0.27.3/tasks/#defining-steps)
...
- For information mapping `Workspaces` to `Volumes`, see [Using `Workspace` variables in `TaskRuns`](/vault/Pipelines-v0.27.3/workspaces/#using-workspace-variables-in-taskruns).
...
- For an end-to-end example, see [`Workspaces` in a `TaskRun`](https://github.com/tektoncd/pipeline/tree/release-v0.27.x/examples/v1beta1/taskruns/workspace.yaml).
...
For more information, see the [`LimitRange` code example](https://github.com/tektoncd/pipeline/tree/release-v0.27.x/examples/v1beta1/taskruns/no-ci/limitrange.yaml).
...
[`config/config-defaults.yaml`](https://github.com/tektoncd/pipeline/tree/release-v0.27.x/config/config-defaults.yaml). If you set the global timeout to 0,
...
When a `TaskRun` changes status, [events](/vault/Pipelines-v0.27.3/events/#taskruns) are triggered accordingly.
...
*More information on the inner workings of debug can be found in the [Debug documentation](/vault/Pipelines-v0.27.3/debug/)*
...
See [Building and pushing a Docker image](/vault/Pipelines-v0.27.3/tasks/#building-and-pushing-a-docker-image)
...
this is available as a [TaskRun example](https://github.com/tektoncd/pipeline/tree/release-v0.27.x/examples/v1beta1/taskruns/run-steps-as-non-root.yaml).
