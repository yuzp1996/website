...
- Optional:
  - [`resources`](#specifying-resources) - **alpha only** Specifies
    [`PipelineResources`](/vault/Pipelines-v0.27.3/resources/) needed or created by the `Tasks` comprising the `Pipeline`.
  - [`tasks`](#adding-tasks-to-the-pipeline):
      - `resources.inputs` / `resource.outputs`
        - [`from`](#using-the-from-parameter) - Indicates the data for a [`PipelineResource`](/vault/Pipelines-v0.27.3/resources/)
...
A `Pipeline` requires [`PipelineResources`](/vault/Pipelines-v0.27.3/resources/) to provide inputs and store outputs
...
- [Using `Workspaces` in `Pipelines`](/vault/Pipelines-v0.27.3/workspaces/#using-workspaces-in-pipelines)
- The [`Workspaces` in a `PipelineRun`](https://github.com/tektoncd/pipeline/tree/release-v0.27.x/examples/v1beta1/pipelineruns/workspaces.yaml) code example
- The [variables available in a `PipelineRun`](/vault/Pipelines-v0.27.3/variables/#variables-available-in-a-pipeline), including `workspaces.<name>.bound`.
...
by using [variable substitution](/vault/Pipelines-v0.27.3/variables/#variables-available-in-a-pipeline).
...
## Adding `Tasks` to the `Pipeline`
 ...
 Your `Pipeline` definition must reference at least one [`Task`](/vault/Pipelines-v0.27.3/tasks/).
...
You can also provide [`Parameters`](/vault/Pipelines-v0.27.3/tasks/#specifying-parameters):
...
`"true"` in the `feature-flags` configmap, see [`install.md`](/vault/Pipelines-v0.27.3/install/#customizing-the-pipelines-controller-behavior)**
...
See the [Service Account](/vault/Pipelines-v0.27.3/pipelineruns/#specifying-custom-serviceaccount-credentials) section
...
so long as the artifact adheres to the [contract](/vault/Pipelines-v0.27.3/tekton-bundle-contracts/).
...
The declared `when` expressions are evaluated before the `Task` is run. If all the `when` expressions evaluate to `True`, the `Task` is run. If any of the `when` expressions evaluate to `False`, the `Task` is not run and the `Task` is listed in the [`Skipped Tasks` section of the `PipelineRunStatus`](/vault/Pipelines-v0.27.3/pipelineruns/#monitoring-execution-status).
...
For an end-to-end example, see [PipelineRun with `when` expressions](https://github.com/tektoncd/pipeline/tree/release-v0.27.x/examples/v1beta1/pipelineruns/pipelinerun-with-when-expressions.yaml).
...
in [`config/config-feature-flags.yaml`](/vault/Pipelines-v0.27.3/install/#customizing-the-pipelines-controller-behavior) defaults to "false".
...
to `Task` using the `scope-when-expressions-to-task` field in [`config/config-feature-flags.yaml`](/vault/Pipelines-v0.27.3/install/#customizing-the-pipelines-controller-behavior)
...
**Note:** `Conditions` are [deprecated](/vault/Pipelines-v0.27.3/deprecations/), use [`when` expressions](#guard-task-execution-using-when-expressions) instead.
...
[`Condition`](/vault/Pipelines-v0.27.3/conditions/) resources. The declared `Conditions` are run before the `Task` is run.
...
In this example, `is-master-branch` refers to a [Condition](/vault/Pipelines-v0.27.3/conditions/) resource. The `deploy`
...
**Note:** If you do not specify a `Timeout` value, Tekton instead honors the timeout for the [`PipelineRun`](/vault/Pipelines-v0.27.3/pipelineruns/#configuring-a-pipelinerun).
...
See the [complete list of variable substitutions for Pipelines](/vault/Pipelines-v0.27.3/variables/#variables-available-in-a-pipeline)
and the [list of fields that accept substitutions](/vault/Pipelines-v0.27.3/variables/#fields-that-accept-variable-substitutions).
...
Tasks can emit [`Results`](/vault/Pipelines-v0.27.3/tasks/#emitting-results) when they execute. A Pipeline can use these
...
[variable substitution](/vault/Pipelines-v0.27.3/variables/#variables-available-in-a-pipeline) - one `Task` emits
...
For an end-to-end example, see [`Task` `Results` in a `PipelineRun`](https://github.com/tektoncd/pipeline/tree/release-v0.27.x/examples/v1beta1/pipelineruns/task_results_example.yaml).
...
For an end-to-end example, see [`Results` in a `PipelineRun`](https://github.com/tektoncd/pipeline/tree/release-v0.27.x/examples/v1beta1/pipelineruns/pipelinerun-results.yaml).
...
- _resource dependencies_:
  - [`from`](#using-the-from-parameter) clauses on the [`PipelineResources`](/vault/Pipelines-v0.27.3/resources/) used by each `Task`
...
taskSpec](/vault/Pipelines-v0.27.3/taskruns/#specifying-the-target-task). For example:
...
`finally` tasks can specify [workspaces](/vault/Pipelines-v0.27.3/workspaces/) which `PipelineTasks` might have utilized
...
Similar to `tasks`, you can specify [`Parameters`](/vault/Pipelines-v0.27.3/tasks/#specifying-parameters) in `finally` tasks:
...
| one or more `PipelineTasks` [skipped](/vault/Pipelines-v0.27.3/conditions/) and rest successful | `true` | `Completed` |
...
| one or more `PipelineTask` [skipped](/vault/Pipelines-v0.27.3/conditions/) and rest successful | all `finally` tasks successful | `true` | `Completed` |
| one or more `PipelineTask` [skipped](/vault/Pipelines-v0.27.3/conditions/) and rest successful | one or more failure of `finally` tasks | `false` | `Failed` |
...
Please refer to the [table](/vault/Pipelines-v0.27.3/pipelineruns/#monitoring-execution-status) under Monitoring Execution Status to learn about
...
For an end-to-end example, see [`status` in a `PipelineRun`](https://github.com/tektoncd/pipeline/tree/release-v0.27.x/examples/v1beta1/pipelineruns/pipelinerun-task-execution-status.yaml).
...
For an end-to-end example, see [`$(tasks.status)` usage in a `Pipeline`](https://github.com/tektoncd/pipeline/tree/release-v0.27.x/examples/v1beta1/pipelineruns/pipelinerun-task-execution-status.yaml).
...
[developer guide for custom controllers supporting `taskSpec`.](/vault/Pipelines-v0.27.3/runs/#developer-guide-for-custom-controllers-supporting-spec)
...
If a custom task supports [`parameters`](/vault/Pipelines-v0.27.3/tasks/#parameters), you can use the
...
If the custom task supports it, you can provide [`Workspaces`](/vault/Pipelines-v0.27.3/workspaces/#using-workspaces-in-tasks) to share data with the custom task.
