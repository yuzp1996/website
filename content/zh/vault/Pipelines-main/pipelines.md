...
- Optional:
  - [`resources`](#specifying-resources) - **alpha only** Specifies
    [`PipelineResources`](/vault/Pipelines-main/resources/) needed or created by the `Tasks` comprising the `Pipeline`.
  - [`tasks`](#adding-tasks-to-the-pipeline):
      - `resources.inputs` / `resource.outputs`
        - [`from`](#using-the-from-parameter) - Indicates the data for a [`PipelineResource`](/vault/Pipelines-main/resources/)
...
A `Pipeline` requires [`PipelineResources`](/vault/Pipelines-main/resources/) to provide inputs and store outputs
...
- The [`Workspaces` in a `PipelineRun`](https://github.com/tektoncd/pipeline/tree/main/examples/v1beta1/pipelineruns/workspaces.yaml) code example
...
`"true"` in the `feature-flags` configmap, see [`install.md`](https://github.com/tektoncd/pipeline/tree/main/docs/install.md#customizing-the-pipelines-controller-behavior)**
...
so long as the artifact adheres to the [contract](https://github.com/tektoncd/pipeline/tree/main/docs/tekton-bundle-contracts.md).
...
For an end-to-end example, see [PipelineRun with `when` expressions](https://github.com/tektoncd/pipeline/tree/main/examples/v1beta1/pipelineruns/pipelinerun-with-when-expressions.yaml).
...
in [`config/config-feature-flags.yaml`](https://github.com/tektoncd/pipeline/tree/main/docs/install.md#customizing-the-pipelines-controller-behavior) defaults to "false".
...
to `Task` using the `scope-when-expressions-to-task` field in [`config/config-feature-flags.yaml`](https://github.com/tektoncd/pipeline/tree/main/docs/install.md#customizing-the-pipelines-controller-behavior)
...
**Note:** `Conditions` are [deprecated](https://github.com/tektoncd/pipeline/tree/main/docs/deprecations.md), use [`when` expressions](#guard-task-execution-using-when-expressions) instead.
...
[`Condition`](/vault/Pipelines-main/conditions/) resources. The declared `Conditions` are run before the `Task` is run.
...
In this example, `is-master-branch` refers to a [Condition](/vault/Pipelines-main/conditions/) resource. The `deploy`
...
For an end-to-end example, see [`Task` `Results` in a `PipelineRun`](https://github.com/tektoncd/pipeline/tree/main/examples/v1beta1/pipelineruns/task_results_example.yaml).
...
For an end-to-end example, see [`Results` in a `PipelineRun`](https://github.com/tektoncd/pipeline/tree/main/examples/v1beta1/pipelineruns/pipelinerun-results.yaml).
...
- _resource dependencies_:
  - [`from`](#using-the-from-parameter) clauses on the [`PipelineResources`](/vault/Pipelines-main/resources/) used by each `Task`
...
| one or more `PipelineTasks` [skipped](/vault/Pipelines-main/conditions/) and rest successful | `true` | `Completed` |
...
| one or more `PipelineTask` [skipped](/vault/Pipelines-main/conditions/) and rest successful | all `finally` tasks successful | `true` | `Completed` |
| one or more `PipelineTask` [skipped](/vault/Pipelines-main/conditions/) and rest successful | one or more failure of `finally` tasks | `false` | `Failed` |
...
For an end-to-end example, see [`status` in a `PipelineRun`](https://github.com/tektoncd/pipeline/tree/main/examples/v1beta1/pipelineruns/pipelinerun-task-execution-status.yaml).
...
For an end-to-end example, see [`$(tasks.status)` usage in a `Pipeline`](https://github.com/tektoncd/pipeline/tree/main/examples/v1beta1/pipelineruns/pipelinerun-task-execution-status.yaml).
...
[developer guide for custom controllers supporting `taskSpec`.](https://github.com/tektoncd/pipeline/tree/main/docs/runs.md#developer-guide-for-custom-controllers-supporting-spec)
