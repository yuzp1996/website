...
- Optional:
  - [`resources`](#specifying-resources) - Specifies the [`PipelineResources`](/vault/Pipelines-main/resources/) to provision
  ...
  - [`taskRunSpec`](#specifying-taskrunspecs) - Specifies a list of `PipelineRunTaskSpec` which allows for setting `ServiceAccountName` and [`Pod` template](/vault/Pipelines-main/podtemplates/) for each task. This overrides the `Pod` template set for the entire `Pipeline`.
  - [`timeout`](#configuring-a-failure-timeout) - Specifies the timeout before the `PipelineRun` fails.
  - [`podTemplate`](#specifying-a-pod-template) - Specifies a [`Pod` template](/vault/Pipelines-main/podtemplates/) to use as the basis
...
The `Pipeline` in the [`pipelineSpec` example](https://github.com/tektoncd/pipeline/tree/main/examples/v1beta1/pipelineruns/pipelinerun-with-pipelinespec.yaml)
...
In the [`taskSpec` in `pipelineSpec` example](https://github.com/tektoncd/pipeline/tree/main/examples/v1beta1/pipelineruns/pipelinerun-with-pipelinespec-and-taskspec.yaml)
...
- For an end-to-end example, see [`Workspaces` in a `PipelineRun`](https://github.com/tektoncd/pipeline/tree/main/examples/v1beta1/pipelineruns/workspaces.yaml).
