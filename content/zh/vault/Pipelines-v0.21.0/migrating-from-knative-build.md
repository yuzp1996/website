...
* `BuildTemplate` [parameters](/vault/Pipelines-v0.21.0/tasks/#specifying-parameters) now reside inside the `input.params` field of a `Task`. In a
   related change, parameter placeholders, such as `${FOO}`, now follow the format `$(input.parameters.FOO)`. For more information,
   see [Using variable substitution](/vault/Pipelines-v0.21.0/tasks/#using-variable-substitution).

* `Tasks` that ingest inputs from resources must now explicitly specify [`input.resources`](/vault/Pipelines-v0.21.0/tasks/#specifying-resources).
...
* Input resource data is no longer cloned into the `/workspace` directory. Instead, Tekton clones the data into a subdirectory
  ...
  For more information, see [Controlling where resources are mounted](/vault/Pipelines-v0.21.0/resources/#controlling-where-resources-are-mounted).
...
* `TaskRuns` which specify a `PipelineResource` as the value of the `input.resources` field of the invoked `Task`
  ...
  a complete `PipelineResource` definition using the [`resourceSpec`](/vault/Pipelines-v0.21.0/taskruns/#specifying-resources) field.

* Containers that execute the `Steps` in your `Tasks` must now abide by Tekton's [container contract](/vault/Pipelines-v0.21.0/container-contract/)
