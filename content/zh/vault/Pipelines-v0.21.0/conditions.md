...
**Note:** `Conditions` are deprecated, use [WhenExpressions](/vault/Pipelines-v0.21.0/pipelines/#guard-task-execution-using-whenexpressions) instead. 
...
You define each `Condition` within your [`PipelineRun` definition](/vault/Pipelines-v0.21.0/pipelineruns/) and then conditionalize
each desired [`Task`](/vault/Pipelines-v0.21.0/tasks/) in the corresponding `Pipeline` definition. 
...
**Note:** [Labels](/vault/Pipelines-v0.21.0/labels/) and annotations specified in the `Condition's` metadata are automatically
...
corresponding `Task` in your `Pipeline` can execute. This field must specify a [`Step`](/vault/Pipelines-v0.21.0/tasks/#steps). 
...
You can specify input [`PipelineResources`](/vault/Pipelines-v0.21.0/resources/) in your `Condition` definition to 
...
- You can access them via [variable substitution](/vault/Pipelines-v0.21.0/resources/#variable-substitution).
- You can use the `targetPath` field to [specify a mount point](/vault/Pipelines-v0.21.0/resources/#controlling-where-resources-are-mounted).
