...
- Optional:
  - [`resources`](#specifying-resources) - **alpha only** Specifies
    [`PipelineResources`](/vault/Pipelines-v0.21.0/resources/) needed or created by the `Tasks` comprising the `Pipeline`.
  - [`tasks`](#adding-tasks-to-the-pipeline):
      - `resources.inputs` / `resource.outputs`
        - [`from`](#using-the-from-parameter) - Indicates the data for a [`PipelineResource`](/vault/Pipelines-v0.21.0/resources/)
...
A `Pipeline` requires [`PipelineResources`](/vault/Pipelines-v0.21.0/resources/) to provide inputs and store outputs
...
- [Using `Workspaces` in `Pipelines`](/vault/Pipelines-v0.21.0/workspaces/#using-workspaces-in-pipelines)
- The [`Workspaces` in a `PipelineRun`](https://github.com/tektoncd/pipeline/tree/release-v0.21.x/examples/v1beta1/pipelineruns/workspaces.yaml) code example
- The [variables available in a `PipelineRun`](/vault/Pipelines-v0.21.0/variables/#variables-available-in-a-pipeline), including `workspaces.<name>.bound`.
...
by using [variable substitution](/vault/Pipelines-v0.21.0/variables/#variables-available-in-a-pipeline).
...
## Adding `Tasks` to the `Pipeline`
 ...
 Your `Pipeline` definition must reference at least one [`Task`](/vault/Pipelines-v0.21.0/tasks/).
...
You can also provide [`Parameters`](/vault/Pipelines-v0.21.0/tasks/#specifying-parameters):
...
`"true"` in the `feature-flags` configmap, see [`install.md`](/vault/Pipelines-v0.21.0/install/#customizing-the-pipelines-controller-behavior)**
...
so long as the artifact adheres to the [contract](/vault/Pipelines-v0.21.0/tekton-bundle-contracts/).
...
The declared `WhenExpressions` are evaluated before the `Task` is run. If all the `WhenExpressions` evaluate to `True`, the `Task` is run. If any of the `WhenExpressions` evaluate to `False`, the `Task` is not run and the `Task` is listed in the [`Skipped Tasks` section of the `PipelineRunStatus`](/vault/Pipelines-v0.21.0/pipelineruns/#monitoring-execution-status).
...
For an end-to-end example, see [PipelineRun with WhenExpressions](https://github.com/tektoncd/pipeline/tree/release-v0.21.x/examples/v1beta1/pipelineruns/pipelinerun-with-when-expressions.yaml).
...
**Note:** `Conditions` are [deprecated](/vault/Pipelines-v0.21.0/deprecations/), use [`WhenExpressions`](#guard-task-execution-using-whenexpressions) instead.
...
[`Condition`](/vault/Pipelines-v0.21.0/conditions/) resources. The declared `Conditions` are run before the `Task` is run.
...
In this example, `is-master-branch` refers to a [Condition](/vault/Pipelines-v0.21.0/conditions/) resource. The `deploy`
...
**Note:** If you do not specify a `Timeout` value, Tekton instead honors the timeout for the [`PipelineRun`](/vault/Pipelines-v0.21.0/pipelineruns/#configuring-a-pipelinerun).
...
See the [complete list of variable substitutions for Pipelines](/vault/Pipelines-v0.21.0/variables/#variables-available-in-a-pipeline)
and the [list of fields that accept substitutions](/vault/Pipelines-v0.21.0/variables/#fields-that-accept-variable-substitutions).

## Using `Results`

Tasks can emit [`Results`](/vault/Pipelines-v0.21.0/tasks/#emitting-results) when they execute. A Pipeline can use these
...
[variable substitution](/vault/Pipelines-v0.21.0/variables/#variables-available-in-a-pipeline) - one `Task` emits
...
For an end-to-end example, see [`Task` `Results` in a `PipelineRun`](https://github.com/tektoncd/pipeline/tree/release-v0.21.x/examples/v1beta1/pipelineruns/task_results_example.yaml).
...
For an end-to-end example, see [`Results` in a `PipelineRun`](https://github.com/tektoncd/pipeline/tree/release-v0.21.x/examples/v1beta1/pipelineruns/pipelinerun-results.yaml).
...
- [`from`](#using-the-from-parameter) clauses on the [`PipelineResources`](/vault/Pipelines-v0.21.0/resources/) used by each `Task`
...
taskSpec](/vault/Pipelines-v0.21.0/taskruns/#specifying-the-target-task). For example:
...
Finally tasks can specify [workspaces](/vault/Pipelines-v0.21.0/workspaces/) which `PipelineTasks` might have utilized
...
Similar to `tasks`, you can specify [`Parameters`](/vault/Pipelines-v0.21.0/tasks/#specifying-parameters) in final tasks:
...
| one or more `PipelineTasks` [skipped](/vault/Pipelines-v0.21.0/conditions/) and rest successful | `true` | `Completed` |
...
| one or more `PipelineTask` [skipped](/vault/Pipelines-v0.21.0/conditions/) and rest successful | all final tasks successful | `true` | `Completed` |
| one or more `PipelineTask` [skipped](/vault/Pipelines-v0.21.0/conditions/) and rest successful | one or more failure of final tasks | `false` | `Failed` |
...
Please refer to the [table](/vault/Pipelines-v0.21.0/pipelineruns/#monitoring-execution-status) under Monitoring Execution Status to learn about
...
For an end-to-end example, see [`status` in a `PipelineRun`](https://github.com/tektoncd/pipeline/tree/release-v0.21.x/examples/v1beta1/pipelineruns/pipelinerun-task-execution-status.yaml).
...
If a custom task supports [`parameters`](/vault/Pipelines-v0.21.0/tasks/#parameters), you can use the
...
If the custom task supports it, you can provide [`Workspaces`](/vault/Pipelines-v0.21.0/workspaces/#using-workspaces-in-tasks) to share data with the custom task.
