...
A `PipelineRun` allows you to instantiate and execute a [`Pipeline`](/vault/Pipelines-v0.26.0/pipelines/) on-cluster.
...
- Required:
  ...
  - [`spec`][kubernetes-overview] - Specifies the configuration information for
    ...
    - [`pipelineRef` or `pipelineSpec`](#specifying-the-target-pipeline) - Specifies the target [`Pipeline`](/vault/Pipelines-v0.26.0/pipelines/).
- Optional:
  - [`resources`](#specifying-resources) - Specifies the [`PipelineResources`](/vault/Pipelines-v0.26.0/resources/) to provision
  ...
  - [`taskRunSpec`](#specifying-taskrunspecs) - Specifies a list of `PipelineRunTaskSpec` which allows for setting `ServiceAccountName` and [`Pod` template](/vault/Pipelines-v0.26.0/podtemplates/) for each task. This overrides the `Pod` template set for the entire `Pipeline`.
  - [`timeout`](#configuring-a-failure-timeout) - Specifies the timeout before the `PipelineRun` fails.
  - [`podTemplate`](#specifying-a-pod-template) - Specifies a [`Pod` template](/vault/Pipelines-v0.26.0/podtemplates/) to use as the basis
...
The `Pipeline` in the [`pipelineSpec` example](https://github.com/tektoncd/pipeline/tree/v0.26.0/examples/v1beta1/pipelineruns/pipelinerun-with-pipelinespec.yaml)
...
In the [`taskSpec` in `pipelineSpec` example](https://github.com/tektoncd/pipeline/tree/v0.26.0/examples/v1beta1/pipelineruns/pipelinerun-with-pipelinespec-and-taskspec.yaml)
...
in [Pipelines](/vault/Pipelines-v0.26.0/pipelines/#tekton-bundles) or [TaskRuns](/vault/Pipelines-v0.26.0/taskruns/#tekton-bundles).
...
For more information, see [`ServiceAccount`](/vault/Pipelines-v0.26.0/auth/).

[`Custom tasks`](/vault/Pipelines-v0.26.0/pipelines/#using-custom-tasks) may or may not use a service account name.
...
You can specify a [`Pod` template](/vault/Pipelines-v0.26.0/podtemplates/) configuration that will serve as the configuration starting
...
[`Custom tasks`](/vault/Pipelines-v0.26.0/pipelines/#using-custom-tasks) may or may not use a pod template.
...
wide `ServiceAccountName`  and [`podTemplate`](/vault/Pipelines-v0.26.0/podtemplates/) configuration,
for example:
...
- For information on mapping `Workspaces` to `Volumes`, see [Specifying `Workspaces` in `PipelineRuns`](/vault/Pipelines-v0.26.0/workspaces/#specifying-workspaces-in-pipelineruns).
- For a list of supported `Volume` types, see [Specifying `VolumeSources` in `Workspaces`](/vault/Pipelines-v0.26.0/workspaces/#specifying-volumesources-in-workspaces).
- For an end-to-end example, see [`Workspaces` in a `PipelineRun`](https://github.com/tektoncd/pipeline/tree/v0.26.0/examples/v1beta1/pipelineruns/workspaces.yaml).

[`Custom tasks`](/vault/Pipelines-v0.26.0/pipelines/#using-custom-tasks) may or may not use workspaces.
...
For more information, see the [`LimitRange` code example](https://github.com/tektoncd/pipeline/tree/v0.26.0/examples/v1beta1/pipelineruns/no-ci/limitrange.yaml).
...
[`config/config-defaults.yaml`](https://github.com/tektoncd/pipeline/tree/v0.26.0/config/config-defaults.yaml).
...
to a `TaskRun`, the complete [status of the `TaskRun`](/vault/Pipelines-v0.26.0/taskruns/#monitoring-execution-status) and details
...
When a `PipelineRun` changes status, [events](/vault/Pipelines-v0.26.0/events/#pipelineruns) are triggered accordingly.

When a `PipelineRun` has `Tasks` with [WhenExpressions](/vault/Pipelines-v0.26.0/pipelines/#guard-task-execution-using-whenexpressions):
