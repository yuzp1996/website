...
- Standalone `Interceptors`, which are instances of the [`ClusterInterceptor`](/vault/Triggers-main/clusterinterceptors/) Custom Resource Definition (CRD). You specify these `Interceptors` by referencing them,
...
- `ref` - a reference to a [`ClusterInterceptor`](/vault/Triggers-main/clusterinterceptors/) object with the following fields:
...
Tekton Triggers ships with the `ClusterInterceptor` Custom Resource Definition (CRD), which you can use to implement custom `Interceptors`. See [`ClusterInterceptors`](/vault/Triggers-main/clusterinterceptors/) for more information.
