...
It exposes an addressable sink that receives incoming event and specifies one or more [`Triggers`](/vault/Triggers-v0.16.1/triggers/).
The sink is a Kubernetes service running the sink logic inside a dedicated Pod.

Each `Trigger`, in turn, allows you to specify one or more [`TriggerBindings`](/vault/Triggers-v0.16.1/triggerbindings/) that allow
you to extract fields and their values from event payloads, and one or more [`TriggerTemplates`](/vault/Triggers-v0.16.1/triggertemplates/)
...
or more [`Interceptors`](/vault/Triggers-v0.16.1/interceptors/).
...
- A Kubernetes ClusterRoleBinding with `tekton-triggers-eventlistener-clusterroles` clusterrole.
  ...
  You can checkout an example [here](https://github.com/tektoncd/triggers/tree/release-v0.16.x/examples/rbac.yaml).
...
- `triggerRef` - (optional) a reference to an external [`Trigger`](/vault/Triggers-v0.16.1/triggers/)
...
For more information, see [`Interceptors`](/vault/Triggers-v0.16.1/interceptors/).
...
An `EventListener` times out if it cannot process an event request within a timeout specified in [controller.yaml](https://github.com/tektoncd/triggers/tree/release-v0.16.x/config/controller.yaml). The timeouts are as follows:
...
and our [TLS configuration example](https://github.com/tektoncd/triggers/tree/release-v0.16.x/examples/eventlistener-tls-connection/README.md) for more information.
...
Tekton Triggers automatically creates and populates this `ConfigMap` with default values described in [config-logging.yaml](https://github.com/tektoncd/triggers/tree/release-v0.16.x/config/config-logging.yaml).
...
You can configure metrics using the [`config-observability-triggers` config map](https://github.com/tektoncd/triggers/tree/release-v0.16.x/config/config-observability.yaml) in the `EventListener` namespaces.
...
Use the Tekton [`create-ingress`](/vault/Triggers-v0.16.1/getting-started/create-ingress.yaml) task to configure an `Ingress` object using self-signed certificates.
...
- How to securely control how each `EventListener` and its associated objects, such as [`Interceptors`](/vault/Triggers-v0.16.1/interceptors/),
