...
**Note:** `Conditions` are deprecated, use [`when` expressions](/docs/pipelines/pipelines/#guard-task-execution-using-when-expressions) instead. 
...
You define each `Condition` within your [`PipelineRun` definition](/docs/pipelines/pipelineruns/) and then conditionalize
each desired [`Task`](/docs/pipelines/tasks/) in the corresponding `Pipeline` definition. 
...
**Note:** [Labels](/docs/pipelines/labels/) and annotations specified in the `Condition's` metadata are automatically
...
corresponding `Task` in your `Pipeline` can execute. This field must specify a [`Step`](/docs/pipelines/tasks/#steps). 
...
You can specify input [`PipelineResources`](/docs/pipelines/resources/) in your `Condition` definition to 
...
- You can access them via [variable substitution](/docs/pipelines/resources/#variable-substitution).
- You can use the `targetPath` field to [specify a mount point](/docs/pipelines/resources/#controlling-where-resources-are-mounted).
