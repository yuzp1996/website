...
- Standalone `Interceptors`, which are instances of the [`ClusterInterceptor`](/docs/triggers/clusterinterceptors/) Custom Resource Definition (CRD). You specify these `Interceptors` by referencing them,
...
- `ref` - a reference to a [`ClusterInterceptor`](/docs/triggers/clusterinterceptors/) object with the following fields:
...
For more information, see our [example](https://github.com/tektoncd/triggers/tree/v0.17.0/examples/github) of using this `Interceptor`.
...
Tekton Triggers ships with the `ClusterInterceptor` Custom Resource Definition (CRD), which you can use to implement custom `Interceptors`. See [`ClusterInterceptors`](/docs/triggers/clusterinterceptors/) for more information.
